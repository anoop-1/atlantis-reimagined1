...
export const metadata: Metadata = {
  ...
  description: "Practical guide for pipeline integrity engineers: how to apply coating disbondment in real inspection work, what auditors expect, and where most programs f...",
  keywords: ["coating disbondment","CIPS DCVG","cathodic protection survey","pipeline coating"],
  alternates: { canonical: "https://pipeline-integrity-guide.vercel.app/case-studies/pipeline-coating-disbondment-detection-tools" },
  openGraph: {
    title: "Pipeline Coating Disbondment: Detection Tools and Field Workflow",
    description: "Practical guide for pipeline integrity engineers: how to apply coating disbondment in real inspection work, what auditors expect, and where most programs f...",
    type: 'article',
    url: "https://pipeline-integrity-guide.vercel.app/case-studies/pipeline-coating-disbondment-detection-tools",
    siteName: "Pipeline Integrity Guide",
    locale: 'en_US',
    publishedTime: "2025-09-02T00:00:00.000Z",
    modifiedTime: "2025-09-02T00:00:00.000Z",
    authors: ["Hector Alvarez, NACE-certified Pipeline Inspector"],
  },
};
...
const jsonLd = {
  ...
  "@type": "Article",
  "headline": "Pipeline Coating Disbondment: Detection Tools and Field Workflow",
  "description": "Practical guide for pipeline integrity engineers: how to apply coating disbondment in real inspection work, what auditors expect, and where most programs f...",
  "author": {
    "@type": "Person",
    "name": "Hector Alvarez, NACE-certified Pipeline Inspector"
  },
  "publisher": {
    "@type": "Organization",
  ...
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://pipeline-integrity-guide.vercel.app/case-studies/pipeline-coating-disbondment-detection-tools"
  },
  "keywords": "coating disbondment, CIPS DCVG, cathodic protection survey, pipeline coating"
};
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 prose prose-slate prose-lg">
      ...
      <nav className="breadcrumb text-sm text-gray-500 mb-6">
        ...
        <span> / </span>
        <span>Pipeline Coating Disbondment: Detection Tools and Field Workflow</span>
      </nav>
      <h1>Pipeline Coating Disbondment: Detection Tools and Field Workflow</h1>
      <p className="text-sm text-gray-500 mb-8">By Hector Alvarez, NACE-certified Pipeline Inspector &middot; Published 2025-09-02</p>
      <div dangerouslySetInnerHTML={{ __html: `<p>This article walks through the practical decisions pipeline integrity engineers, midstream operators, regulators need to make when working on pipeline integrity & inspection. The goal is not theory — it is the kind of working knowledge an experienced inspector or engineer would share over a coffee on a job site, with enough rigor to point back to code and standard references when those decisions get challenged in audit. Throughout the piece we focus on the parts of coating disbondment that turn into real schedule or budget pain when they go wrong.</p><h2>Why this comes up so often in the field</h2>
<p>Ask any pipeline integrity engineers how often coating disbondment drives schedule pain and you will hear the same answer: more often than the project plan ever assumes. Part of the reason is that this kind of inspection sits at the intersection of multiple owners — operations, integrity, fabrication, and QA — and the ownership boundaries are rarely clean. When a problem surfaces, it is usually too late to plan; the team is already in execution mode and looking for guidance.</p>
...
<ul>
<li><strong>The decision points</strong> that drive most coating disbondment programs disputes between QA, integrity, and operations.</li>
...
<p>Below is the working matrix our team uses on day one of any coating disbondment engagement. It is deliberately simple because complexity here costs more than it saves.</p>
...
<p>The textbook approach to coating disbondment usually points engineers at one or two techniques. The field reality is messier; the technique that ranks best on a comparison chart is often not the one that fits the access, the schedule, or the inspector skill mix actually available on the day. We walk through three patterns we see repeatedly.</p>
...
<p>Auditors do not argue with conclusions; they argue with the evidence behind them. For any defensible coating disbondment report, the evidence packet needs to include at minimum:</p>
<ul>
...
<li><strong>Letting one inspector own the data interpretation alone.</strong> Interpretation of coating disbondment should always have a second reviewer for any indication that drives a fitness or repair decision.</li>
...
<p>If we had to summarize coating disbondment in one line it would be this: <strong>the technique matters less than the decision discipline around it.</strong> Teams that consistently choose the right technique are usually teams that have invested in writing down their decision rationale, qualifying their procedures with care, and keeping their inspectors current. Equipment and software change every few years; that discipline does not.</p><h3>How coating disbondment fits into the bigger picture</h3>
<p>It is easy to study coating disbondment as an isolated subject — most courses do exactly that — but the engineering value only appears when you place coating disbondment alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.nrc.gov/reactors/operating/ops-experience.html" rel="noopener nofollow">NRC operating experience reports</a> to align their practice with what is already published in the wider community. For most pipeline integrity engineers the question is not "what is coating disbondment?" but "where does coating disbondment sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in coating disbondment, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
<h3>Operator behaviours that actually move the needle on CIPS DCVG</h3>
<p>Three behaviours we see at high-performing operators consistently show up in CIPS DCVG programs that are working: written rationale for every off-procedure decision; a standing peer-review for indications that drive a fitness call; and a quarterly close-out review where the integrity engineer and the inspection lead sit down with the raw data, not just the summary. None of these behaviours require new hardware. All three improve outcomes more than another tool purchase usually does.</p>
<p>What we do not see at high-performing operators is over-reliance on a single inspector or a single contractor. Cross-checking — even informally — is the cheapest insurance you can buy on a CIPS DCVG program.</p>
<h3>Documentation patterns worth borrowing</h3>
<p>The best cathodic protection survey documentation packages we have seen share three patterns:</p>
<ul>
...
<h3>How CIPS DCVG fits into the bigger picture</h3>
<p>It is easy to study CIPS DCVG as an isolated subject — most courses do exactly that — but the engineering value only appears when you place CIPS DCVG alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.nrc.gov/reactors/operating/ops-experience.html" rel="noopener nofollow">NRC operating experience reports</a> to align their practice with what is already published in the wider community. For most pipeline integrity engineers the question is not "what is CIPS DCVG?" but "where does CIPS DCVG sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in CIPS DCVG, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
<h3>Operator behaviours that actually move the needle on cathodic protection survey</h3>
<p>Three behaviours we see at high-performing operators consistently show up in cathodic protection survey programs that are working: written rationale for every off-procedure decision; a standing peer-review for indications that drive a fitness call; and a quarterly close-out review where the integrity engineer and the inspection lead sit down with the raw data, not just the summary. None of these behaviours require new hardware. All three improve outcomes more than another tool purchase usually does.</p>
<p>What we do not see at high-performing operators is over-reliance on a single inspector or a single contractor. Cross-checking — even informally — is the cheapest insurance you can buy on a cathodic protection survey program.</p>
<h3>Documentation patterns worth borrowing</h3>
<p>The best pipeline coating documentation packages we have seen share three patterns:</p>
<ul>
<li>A short narrative section at the front of every report explaining the inspection objective in plain English. Auditors, regulators, and new hires all read the narrative first.</li>
<li>A consistent indication-numbering scheme that survives across multiple inspection campaigns, so an indication found in 2024 can be tracked through 2026 and 2028 without renaming.</li>
<li>An attached "open items" list with target dates, ownership, and the trigger that closes each item. This is the single most useful artifact for a turnaround manager.</li>
...
<p>If your current report template is missing one of those, that is the easiest documentation improvement you can make this quarter, and it costs nothing beyond a template update.</p>
<h3>Procurement and contract considerations</h3>
<p>coating disbondment engagements have a recurring procurement pitfall: the scope of work is written in inspection-deliverable language ("ten welds inspected by PAUT"), and the contract success criteria are written in commercial language ("on time, on budget, no NCRs"). Neither captures the engineering outcome the asset owner actually needs, which is a defensible decision about fitness for service. Contracts that explicitly include a deliverable like "decision-grade report with Level III rationale signed and dated" align all three parties — owner, integrity engineer, and contractor — around the same outcome.</p>
