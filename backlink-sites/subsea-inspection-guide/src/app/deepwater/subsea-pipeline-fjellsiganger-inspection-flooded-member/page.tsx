...
export const metadata: Metadata = {
  ...
  description: "Subsea Pipeline FMD explained for subsea engineers — covering flooded member detection, common pitfalls, and the data your audit packet needs.",
  keywords: ["flooded member detection","FMD subsea","subsea pipeline integrity","gamma backscatter"],
  alternates: { canonical: "https://subsea-inspection-guide.vercel.app/deepwater/subsea-pipeline-fjellsiganger-inspection-flooded-member" },
  openGraph: {
    title: "Subsea Pipeline FMD: Detecting Flooded Members With Reliability",
    description: "Subsea Pipeline FMD explained for subsea engineers — covering flooded member detection, common pitfalls, and the data your audit packet needs.",
    type: 'article',
    url: "https://subsea-inspection-guide.vercel.app/deepwater/subsea-pipeline-fjellsiganger-inspection-flooded-member",
    siteName: "Subsea Inspection Guide",
    locale: 'en_US',
    publishedTime: "2025-01-29T00:00:00.000Z",
    modifiedTime: "2025-01-29T00:00:00.000Z",
    authors: ["Captain Olav Berg, IMCA D 04"],
  },
};
...
const jsonLd = {
  ...
  "@type": "Article",
  "headline": "Subsea Pipeline FMD: Detecting Flooded Members With Reliability",
  "description": "Subsea Pipeline FMD explained for subsea engineers — covering flooded member detection, common pitfalls, and the data your audit packet needs.",
  "author": {
    "@type": "Person",
    "name": "Captain Olav Berg, IMCA D 04"
  },
  "publisher": {
    "@type": "Organization",
  ...
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://subsea-inspection-guide.vercel.app/deepwater/subsea-pipeline-fjellsiganger-inspection-flooded-member"
  },
  "keywords": "flooded member detection, FMD subsea, subsea pipeline integrity, gamma backscatter"
};
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 prose prose-slate prose-lg">
      ...
      <nav className="breadcrumb text-sm text-gray-500 mb-6">
        ...
        <span> / </span>
        <span>Subsea Pipeline FMD: Detecting Flooded Members With Reliability</span>
      </nav>
      <h1>Subsea Pipeline FMD: Detecting Flooded Members With Reliability</h1>
      <p className="text-sm text-gray-500 mb-8">By Captain Olav Berg, IMCA D 04 &middot; Published 2025-01-29</p>
      <div dangerouslySetInnerHTML={{ __html: `<p>This article walks through the practical decisions subsea engineers, ROV inspection contractors, integrity managers need to make when working on subsea inspection & integrity. The goal is not theory — it is the kind of working knowledge an experienced inspector or engineer would share over a coffee on a job site, with enough rigor to point back to code and standard references when those decisions get challenged in audit. Throughout the piece we focus on the parts of flooded member detection that turn into real schedule or budget pain when they go wrong.</p><h2>Why this comes up so often in the field</h2>
<p>Ask any subsea engineers how often flooded member detection drives schedule pain and you will hear the same answer: more often than the project plan ever assumes. Part of the reason is that this kind of inspection sits at the intersection of multiple owners — operations, integrity, fabrication, and QA — and the ownership boundaries are rarely clean. When a problem surfaces, it is usually too late to plan; the team is already in execution mode and looking for guidance.</p>
...
<ul>
<li><strong>The decision points</strong> that drive most flooded member detection programs disputes between QA, integrity, and operations.</li>
...
<p>Below is the working matrix our team uses on day one of any flooded member detection engagement. It is deliberately simple because complexity here costs more than it saves.</p>
...
<p>The textbook approach to flooded member detection usually points engineers at one or two techniques. The field reality is messier; the technique that ranks best on a comparison chart is often not the one that fits the access, the schedule, or the inspector skill mix actually available on the day. We walk through three patterns we see repeatedly.</p>
...
<p>Auditors do not argue with conclusions; they argue with the evidence behind them. For any defensible flooded member detection report, the evidence packet needs to include at minimum:</p>
<ul>
...
<li><strong>Letting one inspector own the data interpretation alone.</strong> Interpretation of flooded member detection should always have a second reviewer for any indication that drives a fitness or repair decision.</li>
...
<p>If we had to summarize flooded member detection in one line it would be this: <strong>the technique matters less than the decision discipline around it.</strong> Teams that consistently choose the right technique are usually teams that have invested in writing down their decision rationale, qualifying their procedures with care, and keeping their inspectors current. Equipment and software change every few years; that discipline does not.</p><h3>How flooded member detection fits into the bigger picture</h3>
<p>It is easy to study flooded member detection as an isolated subject — most courses do exactly that — but the engineering value only appears when you place flooded member detection alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.dnv.com/services/structural-and-asset-integrity-services-2293" rel="noopener nofollow">DNV asset integrity services portfolio</a> to align their practice with what is already published in the wider community. For most subsea engineers the question is not "what is flooded member detection?" but "where does flooded member detection sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in flooded member detection, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
<h3>Operator behaviours that actually move the needle on FMD subsea</h3>
<p>Three behaviours we see at high-performing operators consistently show up in FMD subsea programs that are working: written rationale for every off-procedure decision; a standing peer-review for indications that drive a fitness call; and a quarterly close-out review where the integrity engineer and the inspection lead sit down with the raw data, not just the summary. None of these behaviours require new hardware. All three improve outcomes more than another tool purchase usually does.</p>
<p>What we do not see at high-performing operators is over-reliance on a single inspector or a single contractor. Cross-checking — even informally — is the cheapest insurance you can buy on a FMD subsea program.</p>
<h3>Documentation patterns worth borrowing</h3>
<p>The best subsea pipeline integrity documentation packages we have seen share three patterns:</p>
<ul>
...
<h3>How FMD subsea fits into the bigger picture</h3>
<p>It is easy to study FMD subsea as an isolated subject — most courses do exactly that — but the engineering value only appears when you place FMD subsea alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.dnv.com/services/structural-and-asset-integrity-services-2293" rel="noopener nofollow">DNV asset integrity services portfolio</a> to align their practice with what is already published in the wider community. For most subsea engineers the question is not "what is FMD subsea?" but "where does FMD subsea sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in FMD subsea, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
<h3>Operator behaviours that actually move the needle on subsea pipeline integrity</h3>
<p>Three behaviours we see at high-performing operators consistently show up in subsea pipeline integrity programs that are working: written rationale for every off-procedure decision; a standing peer-review for indications that drive a fitness call; and a quarterly close-out review where the integrity engineer and the inspection lead sit down with the raw data, not just the summary. None of these behaviours require new hardware. All three improve outcomes more than another tool purchase usually does.</p>
<p>What we do not see at high-performing operators is over-reliance on a single inspector or a single contractor. Cross-checking — even informally — is the cheapest insurance you can buy on a subsea pipeline integrity program.</p>
...
<li><a href="/deepwater/cathodic-protection-survey-deepwater-cp-monitoring">Cathodic Protection Survey for Deepwater Assets: CP Monitoring Best Practice</a></li>
</ul>
` }} />
    </article>
  );
