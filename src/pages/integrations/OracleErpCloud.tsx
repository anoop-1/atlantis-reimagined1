...
const compareRows = [
    { factor: "Connection method", atlantis: "REST via Oracle Integration Cloud (OIC)", competitor: "Oracle Fusion ERP Cloud REST APIs" },
    { factor: "Supported Oracle versions", atlantis: "Oracle Fusion ERP Cloud (current), Oracle EAM Cloud, Oracle E-Business Suite (legacy via OIC)", competitor: "—" },
    { factor: "Authentication", atlantis: "OAuth2 client credentials via Oracle Identity Cloud Service", competitor: "—" },
    { factor: "Asset master sync", atlantis: "Bi-directional, Oracle master of record", competitor: "—" },
    { factor: "Work order flow", atlantis: "Atlantis raises Service Request, Oracle creates Work Order, status flows back", competitor: "—" },
    { factor: "Cost actuals → Atlantis", atlantis: "On Work Order completion, actual cost + completion date sync back", competitor: "—" },
    { factor: "Deployment time", atlantis: "3–5 weeks typical", competitor: "—" },
    { factor: "Oracle prerequisites", atlantis: "Oracle Integration Cloud subscription, OAuth2 setup, dedicated integration user", competitor: "—" },
    { factor: "Rate limits", atlantis: "Standard OIC throttling — managed by integration scheduler", competitor: "—" },
];
...
export default function OracleErpCloudIntegration() {
    ...
    return (
        <ProductPageLayout
            title="Atlantis Digital Twin — Oracle ERP Cloud Integration: Fusion, EAM, EBS [2026]"
            description="Oracle Fusion ERP Cloud + Oracle EAM Cloud integration: REST via Oracle Integration Cloud (OIC), bi-directional asset master, inspection findings → Service Requests → Work Orders. 3-5 wk deploy."
            ...
            ctaTitle="Plug Atlantis Into Your Oracle ERP"
            ctaSubtitle="Bring your Oracle module list and a sample asset export. We&rsquo;ll show the integration in 30 minutes."
            ...
            bodyChildren={
                <>
                    ...
                    <ul>
                        <li><strong>Week 1:</strong> Oracle / OIC kickoff, integration user provisioning, OAuth2 setup, sandbox connection test.</li>
                        <li><strong>Week 2:</strong> Initial asset master import, hierarchy validation, Service Request create test.</li>
                        <li><strong>Week 3:</strong> User acceptance testing with the integrity team, edge case validation.</li>
                        <li><strong>Week 4–5:</strong> Production cut-over, monitoring live, hypercare.</li>
                    </ul>

                    <h2>OIC tier recommendations</h2>
                    <p>For typical refinery / petrochemical / FPSO scale (5,000–25,000 equipment items, daily inspection capture by 5–20 users), the OIC standard tier is sufficient. For large multi-site operators (100,000+ equipment items, sub-hourly sync windows), the OIC enterprise tier raises rate limits and concurrency caps to comfortable headroom. We&rsquo;ll recommend the right tier during scoping based on your equipment count and update frequency.</p>

                    <h2>Oracle E-Business Suite (legacy) notes</h2>
                    <p>Customers on Oracle E-Business Suite (EBS) — typically 12.x — integrate via OIC&rsquo;s EBS adapter, which abstracts the older PL/SQL and database interfaces. The functional integration is identical to Fusion ERP Cloud from Atlantis&rsquo; perspective. Implementation is slightly longer (5 weeks vs 3) because EBS landscapes typically have more customization and less standard data shapes than Fusion Cloud. We have customers running this in production.</p>
                </>
            }
        />
    );
