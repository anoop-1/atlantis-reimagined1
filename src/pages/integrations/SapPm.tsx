...
const compareRows = [
    ...
    { factor: "SAP-side prerequisites", atlantis: "SAP Gateway active, OAuth2 configured, service user with PM_NOTIFICATION + PM_ORDER + EQUIPMENT_MASTER auths", competitor: "—" },
    { factor: "Certified status", atlantis: "Engineered against SAP Plant Maintenance standard objects per SAP Help documentation", competitor: "—" },
];
...
export default function SapPmIntegration() {
    ...
    return (
        <ProductPageLayout
            title="Atlantis Digital Twin — SAP PM Integration: REST, BAPI, S/4HANA, ECC [2026]"
            description="SAP PM integration for Atlantis Digital Twin: REST via SAP Gateway, BAPI/IDoc via PI/PO. Bi-directional equipment master, inspection findings → Notifications → Work Orders. 4-6 wk deploy."
            ...
            ctaTitle="Plug Atlantis Into Your SAP Landscape"
            ctaSubtitle="Bring your SAP PM org and a sample equipment master export. We&rsquo;ll show the integration in 30 minutes."
            ...
            bodyChildren={
                <>
                    ...
                    <ul>
                        <li><strong>Week 1–2:</strong> SAP Basis kickoff, service user provisioning, OAuth2 setup, sandbox connection test, equipment master schema review.</li>
                        <li><strong>Week 3–4:</strong> Initial equipment master import, asset hierarchy validation, Notification create test in sandbox, integrity team walkthrough.</li>
                        <li><strong>Week 5–6:</strong> User acceptance testing, production cut-over, monitoring dashboards live.</li>
                    </ul>
                    <p>Complex landscapes (multiple SAP systems, federated equipment master, custom Z-fields critical to the integrity workflow) extend to 8 weeks. S/4HANA Cloud deployments are typically faster (4 weeks) because there&rsquo;s no Basis-side custom transport management.</p>

                    <h2>What about SAP S/4HANA Cloud vs on-prem?</h2>
                    <p>Same logical integration, different authentication. S/4HANA Cloud uses OAuth2 client credentials via SAP Cloud Identity. On-prem S/4HANA and ECC 6.0 use SAP Gateway with the same OAuth2 path or basic auth in lower environments. The functional integration is identical from Atlantis&rsquo; perspective.</p>
