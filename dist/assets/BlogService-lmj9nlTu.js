import{b as a}from"./blogs-DfQfmkHR.js";const r="/api/blogs";class s{async getBlogs(){try{const i=await fetch(r);if(!i.ok)throw new Error("Failed to fetch blogs");return(await i.json()).sort((t,n)=>t.order-n.order)}catch(i){return console.error("Error fetching blogs from API, using local data:",i),a.sort((e,t)=>e.order-t.order)}}async getBlogBySlug(i){try{return(await this.getBlogs()).find(t=>t.slug===i)}catch(e){console.error("Error fetching blog by slug:",e);return}}async getBlogById(i){try{return(await this.getBlogs()).find(t=>t.id===i)}catch(e){console.error("Error fetching blog by ID:",e);return}}async addBlog(i){try{const e=await fetch(r,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!e.ok){const t=await e.json();throw new Error(t.error||"Failed to create blog")}return await e.json()}catch(e){throw console.error("Error adding blog:",e),e}}async updateBlog(i,e){try{const t=await fetch(r,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:i,...e})});if(!t.ok){const n=await t.json();throw new Error(n.error||"Failed to update blog")}return await t.json()}catch(t){throw console.error("Error updating blog:",t),t}}async deleteBlog(i){try{const e=await fetch(`${r}?id=${i}`,{method:"DELETE"});if(!e.ok){const t=await e.json();throw new Error(t.error||"Failed to delete blog")}return!0}catch(e){throw console.error("Error deleting blog:",e),e}}async reorderBlogs(i){try{const e=i.map(t=>this.updateBlog(t.id,{order:t.order}));await Promise.all(e)}catch(e){throw console.error("Error reordering blogs:",e),e}}getDefaultBlogs(){return[{id:"1",title:"Understanding Ultrasonic Testing in NDT",slug:"ultrasonic-testing",date:"October 1, 2025",snippet:"Explore how ultrasonic testing helps detect internal flaws in materials without causing damage.",content:`<h2>Understanding Ultrasonic Testing in NDT</h2>
...
</ul>`,author:"Atlantis NDT",order:6,createdAt:"2025-10-12",updatedAt:"2025-10-12"}]}}const c=new s;export{c as b};
